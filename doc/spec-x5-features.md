...
- 論壇回報「App 不能像之前那樣連續評分，評一次刷新一次」，回報者要用多個小號給好友評分。
  回報者之後補充：要給同一樓 100 天然／腹黑時分兩次、每次 50；電腦版評完一次可以馬上再評，App 要「刷新一次」才能再評。
- 調查（多代理 widget 探針）：評分流程從上游至今沒改過，App 也從來沒有「連續評分」功能；評分成功後只是關掉評分頁回到帖子，帖子頁不重新載入、捲動位置不變；切換帳號也不會重載已開啟的帖子頁。
  回報者感覺到的「刷新」最可能是換小號要離開帖子再重開（首頁會整頁重抓），或是評分被論壇拒絕時評分頁轉圈重新載入。
- 線上實測（2026-09-22，兩個測試帳號、維護者的測試帖）：同一帳號對同一樓連續評三次 +1 天然、間隔 3 秒，全部成功，今日剩餘 40 → 39 → 38 → 37。
  TSDM 允許重複評分同一帖，協定上不需要任何刷新。App 流程裡擋住「馬上再評」的是評分成功提示蓋住下一個評分頁的送出鈕（35.3 最後一項），不是論壇。
- 不做專為多帳號批量評分的功能（以某帳號評分、一鍵全帳號評分）：用小號給好友刷糖多半違反論壇規則，App 不應成為刷分工具。只修下面三個對所有人都成立的問題。
- 帖子頁右上選單 →「設定」→ 帳號區「管理帳戶」（簡體介面「管理账户」）可以不離開帖子切換帳號，切完返回帖子不會重載。

### 35.2 論壇端事實（X5 原始碼 `source/app/forum/child/misc/rate.php`、`source/function/function_message.php` 與線上實抓）

- 評分視窗（GET，`inajax`）：`<root><![CDATA[...]]></root>` 裡是 `form#rateform`，分數列 `table.dt` 每列 `score<N>`、評分區間、今日剩餘；「通知作者」是 `input#sendreasonpm` checkbox，平常不勾選。
  用戶組 `reasonpm` 為 2 或 3 時，範本輸出 `checked="checked" disabled="disabled"`，伺服器一律通知作者。
- 評分視窗階段就會擋下的情況（已評過同一帖 `thread_rate_duplicate`、不能評自己、超過評分時限……）：`showmessage` 的 `msgtype` 2，訊息放在 `div.alert_error`，App 原本就會顯示並關閉評分頁。
- 送出（POST）被拒：`msgtype` 3，回應是「原因＋`errorhandle_rate('原因', {...})`」，例如實抓的「抱歉，您的天使币不足，无法评分」（`thread_rate_range_self_invalid`，扣分前就擋下，沒有實際評分）；
  其他還有「抱歉，24 小时评分数超过限制」（`thread_rate_ctrl`）、「请输入正确的分值」（`thread_rate_range_invalid`）、表單驗證串不符（`submit_invalid`，formhash 約每 10^7 秒換一次）。訊息裡的 `'` 會跳脫成 `\'`。
- 送出成功：`showmessage('thread_rate_succeed', dreferer())` → `succeedhandle_rate('轉址', '感谢您的参与，现在将转入评分前页面', {})`，已用實際評分驗證。
- 通知作者：`!empty($_GET['sendreasonpm'])` 就通知。網頁版不勾選時不送這個欄位；App 原本關掉時送 `sendreasonpm=off`，也算非空，所以一樣通知。

### 35.3 App 端行為

- `RateStatus.rateFailed`（新）：送出後被論壇拒絕或請求失敗。表單與已填內容留在原處，把論壇原因（`RateFailedException.reason`，沒有原因時用各語系的 `failedToRate`）顯示在送出鈕上方，不跳通用提示、不換成轉圈。
  同時在背景重抓一次評分視窗，只換掉 `info`（新的 formhash、今日剩餘），表單和原因不動；重抓失敗就忽略。每次送出都有序號，較早一次送出的重抓晚回來時直接丟掉，不會蓋掉較新的評分視窗。再送一次時清掉舊原因。
  `RateStatus.failed` 只用在評分視窗載入失敗，行為不變（跳提示，然後重抓或關頁）。
- `sendreasonpm=on` 只在開關開啟時送出；關閉時不送這個欄位。開關預設仍為開啟，維持修正前「一律通知」的結果；這點和網頁版預設不勾不同。
- `RateWindowInfo.noticeAuthorForced`：`input[name="sendreasonpm"]` 帶 `disabled` 時為真。開關固定開啟、不能切換，副標題顯示「论坛设置为总是通知作者」（三語），送出時帶 `on`。
- `RateRepository` 的 `errorhandle_rate` 正規式改成接受 `\'` 跳脫並還原，原因裡有引號時不再被截斷。
- 「评分成功」提示由評分頁自己顯示，並記下它正顯示在哪個 ScaffoldMessenger 上（`onVisible` 設定、`closed` 清掉）。下一個評分頁開啟後的第一個畫格，只有它還在時才收掉；登入過期、簽到完成這類帶按鈕的其他提示不動。
  它排在別的提示後面時，要等輪到顯示才會觸發 `onVisible`：這時若已有別的評分頁開著（評分成功的那一頁關閉前先把自己移出名單），就在顯示當下收掉。
  之前「评分成功」預設停 4 秒，而且會跟著全域 ScaffoldMessenger 顯示在下一個評分頁上，剛好蓋住送出鈕，約 2.75 秒按不到。
- zh-TW 的 `ratePostPage.failedToRate` 改成繁體「評分失敗」（原本是簡體，現在會常駐顯示在頁面上）。

### 35.4 驗收

- `test_098`（評分視窗與被拒回應是 2026-09-22 用測試帳號實抓，formhash 已遮蔽；成功回應依 X5 原始碼組成，和之後實際評分的回應逐位元組相同；評分視窗被拒的回應依 `msgtype` 2 範本組成）：
  - 解析 X5 評分視窗：分數列、formhash、`noticeAuthorForced`（一般為假，checkbox 帶 disabled 時為真）。拿掉解析會失敗。
  - `RateRepository.rate`：實抓的被拒回應帶出論壇原因；`\'` 跳脫的原因完整還原；成功回應為成功。改回舊正規式會失敗。
  - 評分頁被拒：原因顯示在頁面上、表單與已填分數保留、沒有轉圈、沒有通用失敗提示；評分視窗在背景重抓，下一次送出用新的 formhash；成功後關頁並顯示成功提示。改回舊 bloc 行為或拿掉背景重抓都會失敗。
  - 請求失敗（接在一次被拒之後）：顯示通用原因、上一次的論壇原因消失、表單保留、沒有提示。改成保留舊原因會失敗。
  - 評分視窗被拒（重複評分）：跳出論壇訊息並關頁，沒有送出。
  - 通知作者：開啟時 POST 帶 `sendreasonpm=on`，關閉後 POST 不帶這個欄位。改回送 `off` 會失敗。
  - 論壇強制通知：開關為開、不能切換、顯示說明，POST 帶 `on`。
  - 連續評分：成功後立刻開下一個評分頁，畫面上沒有提示，可直接送出。拿掉開頁收提示會失敗。
  - 帶按鈕的其他提示在評分頁開啟後仍在。改成不分來源一律收掉會失敗。
  - 成功提示排在別的提示後面：別的提示逾時後，成功提示不出現在已開啟的下一個評分頁上，送出鈕點得到。拿掉 `onVisible` 裡的收掉會失敗。
  - 連續兩次被拒、第一次的重抓最後才回來：第三次送出用第二次重抓的 formhash。拿掉序號檢查會失敗。
- 實機：用積分不足或超過 24 小時上限的分數評分，看到論壇原因且表單保留；關掉通知作者評分後對方不應收到通知；連評兩個樓層時送出鈕不被遮住。

## 36. 屏蔽用戶（本機靜默）與論壇提醒屏蔽規則（GitHub #96，2026-09-22）

本節是摘要與驗收；每條規則的細節（識別子、失敗處理、帳號切換、名單讀不到時怎麼辦）見 `doc/user_blocking.md`。

### 36.1 需求

- #96（「希望增加本地屏蔽功能」）：遇到不想往來的人，不想跟他吵、只想不再聯繫；網站上線了屏蔽功能，希望 App 也有。
  補充需求：屏蔽用戶與論壇提醒的「屏蔽」兩種都要；屏蔽要讓對方不知道，以單方面屏蔽為主。
- 所以做成兩個互不相干的功能，介面上分開：**A. App 本機靜默屏蔽用戶**（不用論壇黑名單）、**B. 論壇提醒屏蔽規則**（寫進論壇帳號）。
  管理頁「屏蔽管理」（`/userBlock`，自己的資料頁選單、通知頁右上角）分成「已屏蔽用戶（本機）」與「屏蔽提醒（論壇）」兩區，文字各自說明存在哪裡、對誰生效。
- 不宣稱對方在任何情況下都猜不到。App 能保證的是：本機屏蔽的名單只存在本機、屏蔽與解除不送任何請求、不通知對方、不改變論壇上的互動權限。

### 36.2 論壇端事實

來源：Discuz! X5 原始碼（本機 X5.0 測試站；程式路徑相對於 `source/app/home/child/`，另有註明者除外）與 `test/data` 裡去識別的 TSDM 頁面。TSDM 的外掛與後台設定可能和原始碼不同。

- 論壇黑名單（TSDM 好友頁有「我的黑名单」分頁，`friend_list_empty_x5.html`；`spacecp/friend.php` 的 `op=blacklist`）：加入前先 `friend_delete` 解除好友；
  之後對方加你好友、發私訊（`spacecp/pm.php`）、打招呼、評論、表態都會被 `isblacklist` 擋下並看到 `is_blacklist` 訊息。對方看得出來，互動權限也變了，不符合需求，所以 A 完全不呼叫它。
- 提醒屏蔽連結：提醒頁每則提醒的 `dt` 裡有 `home.php?mod=spacecp&ac=common&op=ignore&authorid=<uid>&type=<type>&handlekey=noticeignore`
  （範本 `template/default/home/space_notice.php`；TSDM 的 `notice_read_x5.html`、`notice_unread_x5.html`、`notice_friend_request_x5.html` 都有）。系統提醒的 `authorid` 是 0。
- 加入規則（`spacecp/common.php` 的 `op=ignore`）：POST `ignoresubmit` 與 `authorid`（該人，或 0＝所有人），論壇把 `type|authorid` 加進隱私設定的 `filter_note`。
- 規則在提醒**產生時**比對（`source/class/helper/helper_notification.php`：`type|0` 或 `type|來源 uid` 在 `filter_note` 裡就不建立提醒）：只影響之後的提醒，已有的提醒不刪除、不改已讀；
  規則存在論壇帳號上，網頁版與其他裝置一樣生效。這不是黑名單，對方不會收到任何東西。
- 列出與移除（`spacecp/privacy.php`）：篩選頁 `ac=privacy&op=filter` 列出勾選中的 `filter_note`；送出 `privacy2submit` 會用送出的勾選**整組取代** `filter_note`、`filter_icon`、`filter_gid`。
  移除一條規則時如果沒有把其他勾選照送，其他設定會一起被清掉。
- 隱私設定頁與屏蔽表單的版面是依 Discuz 原始碼組成的測試頁，不是從 TSDM 線上錄下來的；開發時讀公開頁面曾遇到 Cloudflare 的 403。

### 36.3 App 端行為

A. 本機靜默屏蔽

- 入口：他人資料頁右上角「屏蔽此用戶（本機）」／「解除屏蔽」；提醒卡 ⋮ 選單（該提醒帶來源資料，且作者不是系統、也不是自己）。一律以 uid 判斷，不看名字；不能屏蔽自己，未登入不能用。
- 儲存：settings 表 `userBlockList.<擁有者 uid>`，每個帳號一份，不加新資料表。背景服務與「同步所有帳號」用被同步那個帳號的名單。
  不同步到論壇、網頁版或其他裝置；它在 App 的資料庫裡，所以「匯出資料」的備份會帶著它（匯入是整個資料庫取代）。
  讀不到的名單不當成空的（之後的屏蔽不會把原名單蓋掉），讀取期間與失敗時先扣住有作者的內容，並自動重讀、提供重試。
- 屏蔽與解除不送任何請求：不加論壇黑名單、不刪好友、不拒私訊，對方不會收到通知（帖子頁為確認作者可能多讀一次第 1 頁，見下）。
- 隱藏：版塊主題列表、搜尋結果、導讀的完整列表頁、首頁置頂（列表長度與分頁不變，整頁被隱藏也會載入下一頁）；直接開帖只顯示提示與返回／解除屏蔽；
  回覆變成保留樓層號的佔位（只有「解除屏蔽」，沒有「顯示一次」）；同一頁帶論壇 findpost 連結、且被引用樓層的作者已被屏蔽的引用；
  提醒（只看提醒自己屏蔽連結裡的作者，系統 0 不隱藏），以及它的推播、未讀數、提醒頁與提醒搜尋。解除後重新出現。
- 帖子頁在名單非空、作者還不知道時先扣住（不顯示標題、樓層、回覆列，不寫瀏覽紀錄），作者依序從第 1 樓、列表留下的 `ThreadAuthorCache`、一次正序第 1 頁請求取得。
  論壇拒絕的頁面（需登入、無權限、主題已刪）與首樓沒有用戶連結（匿名、遊客）的帖子不扣住；屏蔽前就已顯示的帖子在查作者期間與查詢失敗時留在畫面上，確定作者被屏蔽才換成提示。
- 私訊不隱藏，只靜音：不推播、不計入角標（前景、背景服務、同步所有帳號都一樣）；照常接收、照常儲存、留在私訊列表可手動開啟；不改任何已讀狀態。
- 首頁標頭的提醒總數與私訊旗標沒有作者資訊，名單非空或未知時不採用，以免把隱藏的提醒或靜音的對話帶回角標。
- 不過濾（沒有作者 uid，也不用名字猜）：首頁導讀四個模組的列；收藏、瀏覽紀錄、我的帖子的列（打開後由帖子頁處理）；更新前存下的提醒；引用其他頁樓層的內容。管理頁上方的說明寫明這些限制。

B. 論壇提醒屏蔽規則

- 入口只有「通知 → 提醒 → 單則提醒 ⋮ → 屏蔽此類提醒（論壇）…」，因為 type 與作者要從那則提醒自己的屏蔽連結拿；私訊沒有這個入口。
  選「此人的這類提醒」或「所有人的這類提醒」（系統提醒只有後者），提醒類型顯示成文字（未知類型顯示論壇代號）。
  v14 以前存下的提醒沒有來源資料（同步只抓上次之後的提醒，不會補），選單只有一行停用的說明。管理頁只列出與恢復已有規則，並寫明新增入口在哪裡。
- 新增：確認後先讀隱私頁核對登入身分；規則已存在就回報「已有這條規則」、不送出；再讀屏蔽表單、核對欄位（要有 `ignoresubmit`，作者選項正好是該人與 0），送出一次，最後重讀隱私頁確認。送出期間顯示進度，第二條規則會被拒絕（`busy`）。
- 移除：重新讀完整的篩選表單，只拿掉那一條，其他勾選的 `filter_note`／`filter_icon`／`filter_gid` 照送，送出後確認被移除的不在、其他的都還在。
- 寫入一律用字串 map 送出（Android 的 Kotlin client 只收 `Map<String, String>`）；以 301／302／303 回應的寫入照常重讀確認。
  只送往 `https://www.tsdm39.com/home.php`；頁面不合預期就不寫入；不自動重試、不自動寫入。
  失敗原因分開顯示：網路、登入過期、帳號已切換、Cloudflare 攔截、論壇拒絕、頁面不符、規則已不存在、已送出但無法確認。
- 新增或移除的過程中切換帳號，選擇與結果不會套到另一個帳號。

### 36.4 資料庫、版本與升級限制

- 資料庫 schema 13 → 14：`notice` 加兩個可空欄位 `ignore_type`、`author_id`（`from13To14`，`drift_schema_v14.json`）。舊列保持 null，不從正文的連結猜作者。
  欄位已存在就跳過：更新後背景服務的 isolate 可能和 App 同時開檔、同時跑這一步。
- **schema 14 只能是這個遷移**：之後任何 schema 變更從 15 開始。
- 給測試者的 Android 預覽版 1.27.1-blocking.1+80、1.27.1-blocking.3+82（分包 versionCode 80x、82x；1.27.1-blocking.2+81 建置過但沒有公開）已經是 schema 14。之後覆蓋安裝在它們上面的正式版：
  - build number 必須**大於**最後一個已發布預覽版（目前是 82，所以至少 83）。不能只是相等：versionCode 是 build number × 10 加 ABI 碼（arm64 3、armeabi 2、universal 9），同號的正式版會低於那個預覽版的 universal apk。Android 拒絕較低的 versionCode，退回只能解除安裝，會失去所有本機帳號、cookie 與屏蔽名單；
...
- 預覽版由「Test build」workflow 手動觸發（`build_android`、`build_name`、`build_number`，版號在產生程式碼前寫進 `pubspec.yaml`）；放進 draft release 前人工核對簽章與版本。
  每個新預覽版的 build number 要大於所有已發布的預覽版（下一個是 83）；發布後把它加進上面的預覽清單，正式版的下限跟著調高。
  步驟見 `doc/user_blocking.md` C 節。

### 36.5 驗收

- `test_002`：13 → 14 遷移；另一條連線已加好欄位時重跑不失敗；舊提醒保留、來源欄位為 null。
- `test_091`：屏蔽不需要網路；自己、訪客、無效 uid 被拒；各帳號名單獨立、並行屏蔽都保留、重開資料庫還在、讀不懂的項目原樣寫回；引用只信論壇 findpost 連結；
  提醒只看屏蔽連結裡的作者，被隱藏的不算新也不算未讀、解除後重新計入；名單讀不到時扣住有來源的提醒。
- `test_092`：提醒來源解析；列出規則；移除時其他欄位全部保留；寫入是字串 map；轉址回應照常確認；同名欄位不同值拒絕；表單是別的作者、別的類型或別的帳號時不寫入；論壇明確拒絕不報成成功；送出後的連線錯誤是「無法確認」且不重試；
  系統提醒只能對所有人；帶背景腳本的正常頁不被當成 Cloudflare 攔截。
- `test_093`、`test_094`：讀取失敗不覆蓋名單、`watch` 不漏掉讀取期間的屏蔽、同名用戶與不同帳號互不影響；重複的相同儲存鈕可用、系統提醒作者 0、表單 action 不能指到非預設 port。
- `test_095`：確認框開著時切換帳號兩邊都不變；管理頁跟著目前帳號；靜音的私訊卡；提醒搜尋；整頁主題被隱藏仍保留分頁；直接開帖的扣住、查詢失敗重試、匿名首樓、權限頁、已顯示的帖子；回覆佔位。
- `test_096`：同步後的角標、`fresh`、解除後重算、帳號分開、首頁標頭提示的守門（`noticeHintAllowed`、`applyServerHint`）。
- `test_099`：名單列裡有非字串元素時是「讀不到」而不是例外；切帳號時通知頁不停在載入中；引用遮蔽的結果隨樓層保留。
- `test_100`：Cloudflare 以 403／503 回應的攔截頁；從提醒新增規則時提醒列表重載也照常完成、顯示進度與結果；已有規則不報成已更新；類型名稱；名單讀不到時的提示、自動重讀與停止；管理頁；三語 key 與佔位符一致。
- `test_101`：在實際的首頁與通知頁上：沒有屏蔽時標頭提示照舊提高角標，有屏蔽或名單讀不到時不採用；通知頁的計數不含隱藏的提醒與靜音的對話；刪除靜音的對話不會扣掉其他對話的角標。
- `test_102`：版塊列、搜尋結果、首頁置頂只依作者連結的 uid；提醒選單的各個分支與動作；帖子頁的瀏覽紀錄、同頁引用替換、名單讀不到時的扣住與重試。
- `test_103`：被屏蔽的回覆保留樓層號、不建內容，按佔位上的「解除屏蔽」後內容回來。
- `test_090`：背景服務一輪抓到被屏蔽對象的私訊，不推播也不計數。
- 自動測試的論壇頁面都是依原始碼組成或去識別的樣本，沒有對 TSDM 線上送出規則寫入。實機要確認：
  - Android 從提醒 ⋮ 新增一條規則、在管理頁恢復，網頁版的隱私設定跟著改變；
...
## 38. 銀行第一階段（2026-09-26）

- 個人資料頁新增「銀行」，只顯示於自己的資料頁。沿用論壇 `bank_ane:bank` 插件。
- 銀行列表、開戶標記；已開戶銀行的活期摘要、論壇錢包餘額、幣別、利息及注意事項；自己／收到的交易記錄及分頁。日誌的 IP 欄位不保存或顯示。
- 原生交易只有活期存入／取出：輸入正整數金額及銀行密碼，再確認銀行、方向、金額與幣別。小數及超長貼上整筆拒絕，不能變成另一個金額。交易處理中不可連點送出或返回而遺失結果，一般 GET 仍可離頁。
...
- 多帳號以 UID + 請求世代丟棄舊結果，切帳号或換銀行使舊確認失效。密碼不放入 cubit state、不存本機，日誌遮罩涵蓋 `bankpass`。
...
- 開戶、改銀行密碼、定期、匯款、貸款、積分買賣及手動結息保留網頁。完整設計見 `docs/specs/2026-09-26-bank-design.md`。
- 驗證由 `test_120`（解析／表單）、`test_121`（畫面／确认）、`test_122`（帳號與競態）及 `test_028`（日誌遮罩）覆蓋。所有樣本人工去識別化；沒有用真實帳戶執行存取款，線上交易尚待帳號持有人測試。
