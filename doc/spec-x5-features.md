...
- 送出成功：`showmessage('thread_rate_succeed', dreferer())` → `succeedhandle_rate('轉址', '感谢您的参与，现在将转入评分前页面', {})`。
...
- `test_098`（評分視窗與被拒回應是 2026-09-22 用測試帳號實抓，formhash 已遮蔽；成功回應依 X5 原始碼組成，沒有實際評分；評分視窗被拒的回應依 `msgtype` 2 範本組成）：
