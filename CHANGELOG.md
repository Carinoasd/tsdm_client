...
- 通知：私人消息／公共消息列表里的网址不再做成可点的链接。列表只有消息摘要，论坛会把摘要里网址的 `&` 去掉（变成 `mod=viewthreadtid=…`），点这种残缺网址会因为无法辨识而跳到浏览器并落在论坛首页；现在摘要只显示文字，点卡片进入对话／详情后再点完整链接即可在 App 内打开。权限不足、需要登录、帖子不存在等论坛回应本来就在帖子页／版块页内显示，本次补上真实回应的回归测试。(#46)
- 帖子：修复 `[align=center]`／`[align=right]` 的内容在 App 里靠左显示的问题。论坛把对齐标签输出成 `<div align="center">`，此前只有 `<p align>` 会被对齐；现在 `div`、`p` 与旧式 `<center>` 一律按标签对齐，对齐范围只限该区块，后面的正文不受影响。(#47)
...
- 通知：修正回到首页后点通知卡片被判定为「已在通知页」而停在首页的问题——离开页面时没有更新页面位置记录，这个修正已随 1.21.1 发布但当时未写入说明。本版改为直接按当前显示的页面判断是否要打开通知页，并在日志记录通知点击、前后台切换与判断结果，方便追查仍然没有跳转的情况。(#14)
...
- 版块：帖子列表里作者名字旁的圆圈改为显示该作者的论坛头像。列表页面本身不带任何头像，只有作者的 UID，所以头像网址按论坛自己的规则（`static/js/common.js` 里的 `loadAvatar`）用 UID 生成，尺寸与帖子页相同，两边共用同一份头像缓存。匿名或没有 UID 的作者、没有上传过头像的用户，以及头像载入失败时，仍显示名字首字的文字圆圈，不影响列表其余内容。把头像设成外链的用户论坛不会为其生成文件，若 App 在别处（例如帖子页）已缓存过其头像，列表会显示缓存的那张。
