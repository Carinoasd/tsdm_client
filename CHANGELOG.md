...
- 银行：个人资料页新增银行入口，提供活期存取、定期存款及记录、汇款、贷款申请及记录、开户、修改银行密码、销户、手动结息、账户总览和财富排行；定存支取及贷款记录操作依服务器实际提供的表单显示。银行业务使用统一菜单，提交前再次确认账号、表单及条款，交易只发送一次，送出后刷新供核对。论坛停用的积分买卖显示停用原因，无法识别的操作提供网页入口。
- 勋章中心：新增关键字搜索，按勋章名称或说明搜索全部勋章；结果可翻页、刷新时保留搜索词，清除后回到搜索前的分类，切换分类即离开搜索；搜索中、搜索失败与无结果有独立提示，购买与申请操作照常可用。(#122)
- 称号商店：「我的称号」的商店入口改为应用内页面，逐页显示称号图片、名称、价格、论坛说明与天使币余额，标示已拥有；购买前确认并以最新页面重新核对价格，只送出一次，之后以商店状态核对是否已拥有，不会自动佩戴，返回「我的称号」时自动刷新。(#123)

### Fixed

- 银行测试版：修复理财日志表格内的分页行被误判为交易记录；日志读取失败时保留已加载的活期余额。

## [1.28.4] - 2026-09-26

### Added

- 首页：在论坛统计／轮播下方新增「赞助与功能许愿」卡片，整合自愿赞助说明、收款码与 GitHub 功能需求入口；需求由维护者评估是否采纳及时间安排，「关于」页的入口继续保留。(#121)

### Changed

- 检查更新：「公告帖」改为打开论坛上的官方公告帖。(#119)
...
- 通知：通知页与通知详情页的标题栏新增「回到首页」按钮，一键关闭上层页面并切到首页；遇到可能有未保存内容的页面（正在写的回复、编辑帖子等）会停在该页并提示，不会丢弃内容。(#117)

### Fixed

- 每日登录积分：读取首页后补上网页原有的检查新私信请求，让论坛处理每日登录奖励；是否发放与金额由论坛规则决定，不在客户端固定增加积分。(#111)
- 通知：从通知页进入帖子或回复后再点系统通知，会回到已打开的通知页，不再每次叠加一个新的通知页；上层页面有未发送的回复、编辑中的内容或打开的对话框时，仍照旧在上面打开通知页以保留内容。(#117)

## [1.28.2] - 2026-09-25

### Added

- 好友申请：支持在 App 内批准好友申请。(#102、#104)
...
- 平板：修复双击导航返回顶部／刷新，以及解析页面的浏览器打开操作。(#106)
- 积分提醒：修复回复后的积分奖励提示不显示，恢复版本更新提醒。(#108)
- Android：内部版本号提高至 93，可从 UI4 与 rewards.1 测试版直接升级。
...
- 屏蔽用户（本机）：在他人的个人资料页右上角，或提醒卡片的 ⋮ 菜单中，可以屏蔽该用户。名单保存在本机、按账号分开：只对这台设备上屏蔽时登录的账号生效，同一设备上的其他账号不会继承，也不会同步到论坛、网页版或其他设备。屏蔽与解除都不向论坛发送请求，不使用论坛黑名单，对方不会收到通知，好友关系与论坛权限不变。被屏蔽者的主题不再出现在版块主题列表、搜索结果、导读的完整列表页与首页置顶中；直接打开其主题时只显示提示，以及返回与「解除屏蔽」按钮；名单不为空时，如果打开的主题页上没有第 1 楼、作者也还不知道（例如从后面的页或回复定位链接打开），会额外读取一次该主题的第 1 页来确认作者；回复变成保留楼层号的占位；回复里引用其同一页楼层、且带有跳转到原帖链接的引用框会被替换；能确认来源的提醒会被隐藏。私信照常接收并保留在列表中，可以手动打开阅读，只是不再弹出系统通知、不计入未读角标。无法判断作者的内容仍可能显示，例如首页导读模块、收藏与浏览记录里的主题标题、更新前收到的提醒，以及引用其他页楼层的内容。在自己的个人资料页菜单或通知页右上角的「屏蔽管理」中查看与解除。(#96)
- 屏蔽提醒（论坛）：在「通知 → 提醒」中单条提醒的 ⋮ 菜单选择「屏蔽此类提醒（论坛）…」，可以不再接收此人或所有人的这类提醒（系统提醒只能选所有人）。规则保存在论坛账号的隐私设置里，网页版与其他设备也会生效；只影响之后产生的提醒，已收到的提醒不会被删除或标为已读。已有规则在「屏蔽管理」中查看与恢复，恢复一条规则时其他隐私筛选设置保持不变。更新前收到的提醒没有来源资料，不能从这些提醒添加规则。这不是论坛黑名单，与本机屏蔽互不影响。(#96)
- Android：双击底栏按键，当前页面返回顶部。感谢 [@Qing-Novel](https://github.com/Qing-Novel)。(#95)

### Fixed

- 版块：修复开启「图片模式」的版块（如原创绘图区）在客户端中看不到帖子的问题，现在会以普通列表显示帖子和置顶帖。
- 评分：论坛拒绝评分时（例如积分不足、超过 24 小时评分上限、分值不正确），直接在提交按钮上方显示论坛给出的原因，评分表单留在原处，不再转圈重新加载评分页、只提示「评分失败」。
- 评分：关闭「通知作者」后不再通知对方；论坛设置为总是通知时，开关会显示说明且无法关闭。
- 评分：连续评分时，上一次的「评分成功」提示不再遮住下一次的提交按钮。

## [1.27.0] - 2026-09-19

### Added

- Android：支持从外部打开论坛的 HTTPS `forum.php` 链接，自动进入对应帖子或版块，返回时保留原页面；支持 `www.tsdm39.com` 与 `tsdm39.com`。如需默认使用客户端打开，请在系统的应用设置中允许打开支持的链接，具体入口依设备而异。感谢 [@Qing-Novel](https://github.com/Qing-Novel)。(#93)
- 勋章中心：新增勋章购买与申请操作，购买前显示确认，人工审核申请支持填写理由；保留右上角的浏览器入口。感谢 [@zCandy0](https://github.com/zCandy0)。(#92)

### Changed

- 个人资料：将「勋章中心」入口移到勋章展示区下方，仅在自己的资料页显示。感谢 [@zCandy0](https://github.com/zCandy0)。(#91)
- Android：调整通知权限设置的图标。感谢 [@Qing-Novel](https://github.com/Qing-Novel)。(#88)

### Fixed

- Android：切换语言后同步更新后台常驻通知语言，先完成设置保存再通知后台服务，避免服务读取到旧语言。感谢 [@Qing-Novel](https://github.com/Qing-Novel)。(#89)
...
- 提醒／私信：两次同步同时进行（App 内与后台服务，或与「同步所有帐号」）时，先发出、后返回的旧结果不再覆盖已存的较新私信摘要，也不再被当成新消息通知；存库改为独占式读改写，另一方等前一方写完再读。(#80)
...
- 私信：在对话页或聊天记录页发送消息后，页面会自动重新读取并显示刚发出的消息；对话页会滚到最新一条。(#76)
...
- 帖子：编辑刚发出的楼层后，帖子页再次会自动刷新并停在该楼层。此前回帖后帖子页会给新楼层做标记，编辑期间页面一旦重建（例如键盘弹出）该标记被移除、卡片被重建，保存成功后的刷新就被跳过。(#76)
- 私信：较慢的旧刷新结果不再覆盖刚送出的消息；聊天记录的刷新与旧页载入依序处理，避免新消息消失。(#81)
...
- 个人资料新增「我的成就」入口，查阅当前帐号的论坛成就内容，支持登录、刷新和错误重试；论坛未提供项目时显示空状态，不推算缺少的完成度，奖励领取保留网页入口。(#44)
- 个人资料新增勋章中心，支持分类、分页和取得条件查阅，完整保留多种积分价格、有效期与申请条件；购买、申请和管理继续在浏览器进行。(#43)
- 首页新增活动总览入口，按论坛首页顺序显示活动链接，支持站内跳转、浏览器打开、刷新与失败重试。(#42)
- 桌面（Windows）：新增系统托盘图标与右键菜单，可查看当前帐号、打开历史、收藏、管理帐户或退出程序；从最小化状态选取页面时先还原并聚焦主窗口，退出前先关闭本地数据库。原始实现来自 [@Qing-Novel](https://github.com/Qing-Novel)。(#59, #61)

### Fixed

- 投票：防采集验证只自动重试 GET／HEAD 读取请求，不再复制 POST 表单重送；投票结果仍以随后重新读取的论坛状态为准。(#41)
- 网络：回复、评分等表单 POST 若收到防采集验证页，不再把验证页当成结果交给页面解析，而是直接回报失败并写入日志；请求不会自动重送，避免重复提交。(#41 后续)
- 桌面：窗口最大化状态会被记住，重开仍是最大化；还原后保留最大化前的普通大小与位置。最小化、全屏与拖动过程中的临时坐标不再覆盖已记住的普通窗口大小与位置，拖动结束时立即写入。启动时先套用大小再套用位置，最后才最大化；Windows 首帧改用 SW_SHOW，不会把还原出来的最大化窗口降回普通窗口。(#54, #33)
- 桌面：全新安装或清空数据后，窗口不再被移到屏幕左上角；只有 App 真的存过位置才会套用（(0, 0) 存过之后仍然有效）。(#54)
- 版块：选择筛选条件（主题分类／类型／时间／排序）后，弹出面板不再放大到整屏并闪出一大片灰白，深色模式下尤其刺眼；面板保持原有大小直到关闭。主题分类面板不再把每个分类列出两次。(#55, #45)
- 个人资料：「勋章中心」入口只在自己的个人资料页显示，查看他人资料时不再出现。(#43 后续)
- 桌面（Windows）：托盘菜单选到已经开在最上层的页面时只还原并聚焦窗口，不再把同一页重复叠上去。(#61 后续)
...
- 版块：置顶／帖子列表显示作者的论坛头像；外链头像可使用 App 已缓存的图片，没有可用头像时保留文字圆圈。修正缓存回退时反复请求图片的问题。
- 浏览记录：新增账号筛选，可查看全部账号或指定账号的记录；支持已移除账号，同名账号以 UID 区分。(#19)

### Fixed

- 帖子：修复置中与靠右内容在 App 内靠左显示的问题，对齐仅作用于对应区块。(#47)
- 通知：私人消息／公共消息列表的摘要不再把残缺网址做成链接；请进入交谈记录或详情后打开完整链接，避免误跳浏览器。(#46)
- 通知：直接按当前显示的页面判断通知点击是否需要跳转，并补充点击与前后台诊断日志。先前页面位置记录的修正已随 1.21.1 发布。(#14)
- Android：升级 Flutter 至 3.41.5，修复视窗缩放或方向切换后可能沿用旧尺寸的问题；保留尺寸诊断日志。(#28)

### Changed

- Android：沿用已合并的 Skia 绘图设置（关闭 Impeller）。回报者确认此设置未消除旋转过场短暂露黑，不能视为该过场问题的修复。(#52)
...
- 编辑器：「我的图片」——把图片网址或图床给的 `[img]` 代码存在本机，表情面板第一个标签即可一键插入；帖子里的图片长按菜单可直接「存到我的图片」；长按缩略图可删除。(#5)
- 首页：改为显示论坛导读页的四个模块——最新热门、最新精华、最新回复、最新发表，每个模块最多 10 条，「更多」进入完整列表；顶部另有「抢沙发」快捷入口；完整列表页显示模块名称，列表为空时给出提示。(#12)

### Changed

- 数据库结构升级到 v13（新增 custom_image 表），覆盖安装自动迁移。

## [1.20.0] - 2026-09-09

处理 GitHub 反馈 #21、#22、#23、#24、#25、#27。

### Added

- 帖子：本地「已回复」标记。回复主题或楼层成功后记录，打开主题时看到自己的楼层也会补记；版块列表的主题卡片和主题页标出已回复的主题，按账户分开，删除账户时一并清除。(#21)
- 分区：每个分区页顶部显示该分区的版主（与网页首页一致），点名字打开对方资料页。(#22)
- 帖子：右上角菜单新增「分享给好友」，从自己的好友列表选人，把帖子标题和链接以私信发送；论坛拒绝时（例如发帖间隔不足 10 秒）显示原因。(#23)
- 账户：登录过期提示。签到、一键同步、通知刷新或首页发现某账户的登录已失效时记录下来，账户管理页对该账户显示「登录已过期」并提供「重新登录」；启动时（以及当前账户在使用中过期时）弹出提示，可点「查看」进入账户管理。(#25)

### Fixed

- 通知：评分留言等纯文字里的网址现在可以点击打开。(#24)
- 首页：统计卡片上下留白一致。(#27)

### Changed

- 数据库结构升级到 v12（新增 replied_thread 表与 cookie.session_expired_at 列），覆盖安装自动迁移。
- 安全漏洞改用本仓库的 Security → Report a vulnerability 私密回报；README 补英文摘要，LICENSE 调整为 GitHub 可识别的 MIT。

## [1.19.2] - 2026-09-09

### Fixed

- 分区：找到「我收藏的版块」在 App 里不显示、必须重新登录才出现的根本原因。论坛在某个会话第一次打开首页时若账号没有收藏版块，会给该会话种下一年有效的 `nofavfid` Cookie，之后对这个会话一律不输出收藏面板，只有在同一会话里收藏才会清掉；在网页收藏后 App 的会话因此一直看不到。现在 App 不再保存、也不再发送这个 Cookie（旧数据加载时一并清理），首页每次都会带出收藏面板。(#1)

## [1.19.1] - 2026-09-09

跟进 1.19.0 发布后的反馈（#1、#13、#14）并处理 #15、#16、#18。

### Added

- 首页：头像菜单新增「管理账户」入口，位于「收藏」之后。(#16)
- 编辑器：桌面端回复编辑器支持 Ctrl+Enter / Alt+Enter 快捷发送，帖子回复、通知回复、私信均适用。(#18)
- 分区：分区页解析后在日志记录分区名称、收藏版块 fid 以及页面里是否带有收藏面板，方便定位「我收藏的版块」不显示的问题。(#1)

### Fixed

- 分区：「我收藏的版块」分页第一次出现时自动选中它，不再停留在原来的分页而让新分页留在可滚动标签栏之外。(#1)
- 分区：登录者识别支持论坛全部六种风格（「默认毛坯」风格没有原来的用户区块，改以每页都有的 `discuz_uid` 作后备），这类用户的页面不再被当成别人的页面多抓一次。(#1)
- 通知：改用新的高重要性通知渠道（`newNoticeChannelV2`，会弹横幅），启动时删除旧渠道；用于排查部分机型（如华为）收不到提醒的问题。(#13)
- 通知：应用未运行时点击通知，启动后直接进入通知页，而不是停在首页。(#14)
- 设置：调试 → 日志页面跟随应用的浅色／深色主题，状态栏图标与 Talker Monitor 页顶栏颜色一致。(#15)

## [1.19.0] - 2026-09-09

处理 GitHub 上的反馈 #1、#2、#3、#4、#6、#8、#9、#10、#13，并补上 Windows 版。

### Added

- 收藏：版块页右上角菜单新增「收藏本版／取消收藏本版」，收藏时可填备注；收藏页分为「帖子」「版块」两个标签，论坛的 `home.php?mod=space&do=favorite&type=forum` 链接直接打开版块标签。(#2)
- 账户：管理账户页每个账户显示「今日已签到／今日未签到」，本次启动自动签到失败的账户附带失败原因；签到成功后立即记录时间，不必等整批结束。(#9)
- 账户：管理账户页长按账户或点击右上角「选择」进入多选，可全选并从本机删除账户，删除前确认且说明不会向论坛退出登录；单个账户对话框的「清除登录记录」改为「删除账户」并需确认，当前账户新增「从本机移除」，不联网也能退出。(#6)
- 设置：「行为」新增「通知权限」与「忽略电池优化」两项（仅 Android），显示当前状态并可直接申请；永久拒绝时引导到应用设置页；在设置里开启自动同步时也会申请通知权限。注意「忽略电池优化」只在应用留在后台时有帮助，不保证后台推送。(#3, #13)
- 设置：Debug 区新增「发送测试通知」，用来分辨「系统或权限拦截」与「没有新消息」。(#13)
- 编辑器：输入 `@`（文首或空白之后）自动弹出提醒用户选单；选单改为底部表，列出自己的好友（头像、用户名、用户组）与官方 @ 名单，可即时筛选，无匹配时仍可直接提醒输入的用户名。(#8)
- 通知：管理账户页的同步按钮和通知页菜单的「同步所有账户」，逐一用各账户自己的登录状态抓取提醒、私信和公共消息并存入本机，进度页按账户显示新消息数与未读数；红点仍只反映当前账户，登录失效或被论坛限流的账户单独标注，不影响其他账户。(#10)
- 桌面：GitHub Releases 新增 Windows 版（`tsdm_client-windows.zip`，解压即用）。

### Fixed

- 分区：在网页上收藏版块（或切换账号、退出登录）后，「我收藏的版块」分区会随首页刷新自动出现／消失，不再需要在分区页手动下拉刷新；修复该分区首次出现或消失后下拉刷新报错、标签页错位的问题；在 App 内收藏或取消收藏版块后分区页也会静默刷新。分区页只信任属于当前登录用户的首页，切换账号或退出后不会短暂显示上一个账号的收藏分区。(#1)
- 签到：自动签到完成的底部提示不再折成两行，去掉了关闭按钮，下滑或点「查看详情」即可关闭。(#4)
- 签到：每个账户的签到时间以其实际签到时刻为准，不再在整批结束时统一覆盖，跨越午夜的签到不会被记成隔天。(#9)
- 账户：论坛会话已失效时「退出登录」也会清除本机登录记录（仅当论坛返回访客页；维护页等无法识别的页面只标记为未登录并保留账户）；离线启动或会话尚未验证时，本机正在使用的账户仍视为当前账户，删除它会真正生效，不再被下一次 Cookie 写回；删除进行中按返回键不再清空选择。(#6)
- 通知：Android 13 及以上未取得通知权限时自动同步的推送会被系统静默丢弃；应用清单明确声明 `POST_NOTIFICATIONS`，开机与每次推送前记录权限状态，推送失败写入日志。当系统不会再弹出权限对话框（Android 12 及以下、已拒绝两次、系统设置里关闭了通知）时，设置页显示「已永久拒绝」并引导到应用设置页。(#13)
- 通知：好友请求提醒在 Discuz! X5 上带有两个 `id` 属性，之前解析不到而被整条略过，现在会正常列在提醒里。
- 编辑器：切换账号或退出后 @ 选单不再沿用上一个账号的官方 @ 名单；含方括号的用户名也能完整成为提及并按官方格式发送。(#8)
- 通知：一键同步途中出错时也会以「完成」收尾并恢复自动同步，单个账户失败不影响其余账户；登录、切换账号与一键同步各自持有自动同步的暂停，不再互相提前恢复。(#10)
- 构建：Windows 版在 Visual Studio 2026 下编译失败（外部插件使用了已弃用的 coroutine 头文件），已修正。

### Changed

- 通知：通知抓取与入库对账抽成共用步骤，一键同步与平时同步的存法完全一致；一键同步完成后通知页直接从本机重建列表，不再多抓一次。(#10)
...
- 性能：楼层、通知、私信、聊天卡片的 HTML 只解析一次并缓存，页面重建（红点变化、键盘弹出等）不再把所有楼层重新解析；楼层列表每项独立绘制层；帖内图片按显示尺寸解码，大图不再整张解码上传 GPU。
...
- app：适配论坛升级到Discuz! X5后的页面结构，修复升级后各页面无法解析的问题。
  - 网络：自动处理论坛开启的防采集校验（打开帖子时需要携带`_dsign`签名），签名按主题缓存。
  - 首页：适配版块列表、主题列表、分页、我的帖子、最新回复的新结构。
  - 帖子：适配楼层信息（作者、头像、勋章、签到、宠物）、评分、回复、购买、编辑表单的新结构，游客浏览时不再空白。
  - 用户：适配提醒、私信、个人资料、积分、签到、用户组、第二牌子页面的新结构。
  - 登录：改为使用Discuz!标准登录流程，仅在服务器要求时显示验证码。
  - 搜索：论坛自定义搜索插件已下线，改用Discuz!内置搜索。

### Changed

- 首页：论坛首页的轮播图与分类置顶帖区块已随论坛升级下线，首页改为显示论坛统计信息；“最新回复”改为打开论坛导读页。
- 通知：论坛自定义的通知接口已下线，改为解析提醒、私信、公共消息页面。
...
- 编辑：导入导出时支持通过粘贴的方式导入BBCode或Quill Delta的内容。
- 编辑：Quill Delta格式支持导入和导出帖子的相关设置项。
  - 现在导出和复制Quill Delta时会包含与帖子相关的设置项，在导入Quill Delta时也会导入帖子设置。
  - 涉及到的设置项有帖子标题、帖子分类、阅读权限、售价、附加选项。
  - 导出Quill Delta时总是会导出上述设置，帖子选项不可用时会导出空选项，保证在导入时兼容。
  - 导入Quill Delta时总是会将包含的设置项覆盖进当前编辑的帖子中，只要设置项的值有效。
  - **BREAKING CHANGE**: 本次变更更改了Quill Delta json的格式，旧版本导出的文件依然可用，但新版本导出的不会被旧版本识别。
- 编辑：在导入导出时添加说明。
  - 现在BBCode格式只保存帖子正文内容，Quill Delta会同时保存帖子正文和帖子设置。
  - 考虑到BBCode parser的稳定性和保证数据安全，建议在保存内容复杂的帖子时以Quill Delta格式保存且不要使用内置的快速回复模板功能。
- 帖子：快速评分模板支持同时使用两个特殊积分属性，现在适应春节活动评分。

### Fixed

- 帖子：修复网页端使用2026新年主题时无法在帖子内回复的问题。

### Changed

- app：将更多底部弹出的sheet改为动态高度，不再固定撑满最大高度。
- app：调整了部分UI以及个人主页的背景图样式。
- deps: 升级至flutter 3.41.1。
- deps: 更新部分依赖大版本。
...
- 分区：现在在帖子卡片上长按或悬停鼠标时会有水波纹特效。
- app：自动解析随大型活动而变化的活动积分名称；支持同时使用两项活动积分。

### Fixed

- 升级：修复检测到新版本后不会弹出升级对话框的问题。
- 编辑：修复解析BBCode时在文档末尾的段落样式中最后一行文字出现在样式外面的问题。
- 编辑：修复解析BBCode时在段落样式后会出现多余空行的问题。

### Changed

- deps：升级至flutter 3.41.0。
