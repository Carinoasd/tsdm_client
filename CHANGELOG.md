...
- 草稿箱：在「我的帖子」新增草稿分页，支持刷新、分页及续编，保存后可直接查看草稿；特殊主题、定时发布及不支持的编辑格式提供网页续编入口。(#125)
...
- 草稿编辑：识别新版论坛标题旁的草稿标记，修复成功提示页误报失败、重复送出及账号切换后的旧表单问题；保存后验证草稿状态，等待审核或无法确认时不误报已存入草稿箱。(#125)
